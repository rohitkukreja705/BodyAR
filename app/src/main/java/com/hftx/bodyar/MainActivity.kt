...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupSidebar() {
        adapter = ClothingAdapter(ClothingRepository.itemsFor(currentCategory), ::onItemTapped)
    ...
    private fun switchCategory(category: ClothingCategory) {
        ...
        adapter.submitList(ClothingRepository.itemsFor(category), selectedId)
    }
    ...
    private fun onItemTapped(item: ClothingItem) {
        when (item.category) {
            ClothingCategory.SHIRT -> {
                ...
                binding.poseOverlay.setSelectedShirt(selectedShirt?.drawableRes)
                adapter.setSelected(selectedShirt?.id)
            }
            ClothingCategory.PANT -> {
                selectedPant = if (selectedPant?.id == item.id) null else item
                binding.poseOverlay.setSelectedPant(selectedPant?.drawableRes)
            ...
            ClothingCategory.SUNGLASSES -> {
                ...
                binding.poseOverlay.setSelectedGlasses(selectedGlasses?.drawableRes)
